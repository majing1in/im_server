...
public class RedisTemplateUtils<K, V> {
     ...
     * @param
     */
    public Boolean setIfAbsent(K key,V value,long time, TimeUnit type) {
     ...
     * @return 出栈的值
     */
    public V rightPop(K key,int time,TimeUnit timeUnit) {
    ...
    public Long taskAdd(K key, V... values) {
        log.info("add {} to {}", values, key);
        return redisTemplate.opsForSet().add(key, values);
    }
    ...
    public Long taskRemove (K key, Object... values) {
        log.info("remove {} from {}", values, key);
        return redisTemplate.opsForSet().remove(key, values);
    }
